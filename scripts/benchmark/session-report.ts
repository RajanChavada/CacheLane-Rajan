#!/usr/bin/env tsx
/**
 * CacheLane Session Report Generator
 *
 * Reads the CacheLane SQLite database and generates a comprehensive report
 * showing cache savings, dollar cost estimates, per-turn timelines, and
 * anomaly detection across all sessions.
 *
 * Usage:
 *   tsx scripts/benchmark/session-report.ts [--json] [--db <path>] [--session <id>]
 */
...
if (values.help) {
  console.log(`
CacheLane Session Report

Usage:
  tsx scripts/benchmark/session-report.ts [options]

Options:
  --db <path>        SQLite database path (default: ~/.cachelane/cachelane.db)
  --session <id>     Report on a specific session (default: all sessions)
  --json             Output as JSON
  --help             Show this help
`);
  process.exit(0);
...
function baselineDollars(tokens: number): number {
  // If no caching existed, you'd pay full input price for everything
...
function generateReport(): FullReport {
  ...
  try {
    ...
    console.error(`Error: Could not open database at ${DB_PATH}`);
  ...
  try {
    ...
    const sessionFilter = values.session
      ? `WHERE session_id = ?`
      : ``;
    const sessionParams = values.session ? [values.session] : [];
...
function renderReport(report: FullReport): string {
  ...
  // ── Per-session detail (for the top sessions or a specific one) ──
  const detailSessions = values.session
    ? report.sessions
    : report.sessions.filter(s => s.turns >= 5).slice(0, 3);
  ...
  for (const s of detailSessions) {
    ...
    // Per-turn timeline (last 20 turns)
...
if (values.json) {
  // Strip turns_data from JSON to keep it manageable
  const jsonReport = {
    ...report,
    sessions: report.sessions.map(({ turns_data, ...rest }) => rest),
  };
